...
def check_auth(headers):
    """Returns True if the request carries a valid API key, False otherwise.
    Auth is skipped entirely when API_SECRET_KEY is not configured (dev mode)."""
    key = _get_api_key()
    if not key:
        return True
...
_rate_store = {}
RATE_WINDOW_S = 1.0  # minimum seconds between requests from the same IP

def check_rate_limit(client_ip):
    """Returns True if the request is within rate limit, False if throttled."""
    now = time.monotonic()
...
def save_data_to_firestore(data):
    """Saves a record to Firestore if there has been a significant change."""
    try:
...
def get_latest_data(device_id=None):
    """Returns the most recent Firestore document, optionally filtered by device_id."""
    try:
        db = get_firestore_client()
        col = db.collection("intellicath_data")
